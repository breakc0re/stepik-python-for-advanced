...
# Дополните приведенный код так, чтобы в переменной result хранился словарь, в котором для каждого владельца
# будут перечислены его собаки. Ключом словаря должен быть кортеж (имя, фамилия, возраст владельца),
...
for pet in pets:
    dog_name, owner_first, owner_last, owner_age = pet
    owner_key = (owner_first, owner_last, owner_age)
    if owner_key in result:
        result[owner_key].append(dog_name)
    else:
        result[owner_key] = [dog_name]
