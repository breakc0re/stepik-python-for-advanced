...
# создания небольшого словаря сленговых программерских выражений, чтобы она потом по запросу возвращала значения
# из этого словаря.
...
slang_dict = [(input().split(': ')) for i in range(int(input()))]
slang_dict = {i[0].lower(): i[1].strip() for i in slang_dict}

for i in [input().lower() for i in range(int(input()))]:
    print(slang_dict.get(i, 'Не найдено'))
